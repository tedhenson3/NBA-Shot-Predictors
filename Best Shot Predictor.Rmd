...
intervals <- predict(best$BestModel, newdata = test, type = 'response', interval = 'confidence')
...
head(intervals)
paste("median difference in lower and upper interval bounds", round(median(intervals[,3] - intervals[,2]), 2))
paste("mean difference in lower and upper interval bounds", round(mean(intervals[,3] - intervals[,2]), 2))
```
...
#Shots deemed low percentage
```{r, echo = FALSE}


badshots <- test[which(intervals[,1] < .4),]
...
#Shots deemed high percentage
```{r, echo = FALSE}

goodshots <- test[which(intervals[,2] > .6),]
